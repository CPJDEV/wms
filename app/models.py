...
class Warehouse(db.Model):
	...
	aisles = db.relationship('Aisle', backref='warehouse', lazy='dynamic')
...
class Aisle(db.Model):
	...
	bins =  db.relationship('Bin', backref='aisle', lazy='dynamic')
	
	def __repr__(self):
		return '<Aisle: {}>'.format(self.name)
...
class Bin(db.Model):
	...
	pallets =  db.relationship('Pallet', backref='bin', lazy='dynamic')
...
class Pallet(db.Model):
	...
	lot_ = db.Column(db.String(60), db.ForeignKey('lot.name'))
	qty_ = db.Column(db.Float)
