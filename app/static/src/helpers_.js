...
function validateMultiBinsEntry(){
	var bins = $(this).val().split(",").length > 1 ? $(this).val().split(",") : $(this).val().split(" ");
	var bins_ = [];
	
	if($("#add_multi_bins").hasClass("active")){

		$.each(bins,function(i,val){
			if(val != "")
				if(bins_.indexOf(val.toUpperCase()) == -1)
					bins_.push(val.toUpperCase());
		});
		
		if(bins_.length > 0){
			var act_bins = bins_.join(",");
			$(this).val(act_bins);
		}
		...
		if($(this).val().split(",").length > 0){
			...
			flash("Feature Error: No data to process. Please input comma / space separated values to add multiple bins.")
			$("input#submit").attr("disabled",true);
		}
	}else{
		if(bins.length > 1){
...
function openSetModule(){
	...
	var module_ = $(this).attr("id");
	var module_val = $(this).attr("data");
	var module_view = $("#"+module_+".views_");
